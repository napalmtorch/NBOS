...
void gui_update_listbox(gui_listbox_t* listbox)
{
    ...
    rect_t itembounds = (rect_t) { .x = bounds.x + 2, .y = bounds.y + 2, .width = bounds.width - 3, .height = font_geth(listbox->base.theme.font) };
...
void gui_draw_listbox(gui_listbox_t* listbox)
{
    ...
    image_rectf(&buff, listbox->base.bounds.x, listbox->base.bounds.y, listbox->base.bounds.width, listbox->base.bounds.height, listbox->base.theme.colors[THEME_BG]);

    if (listbox->base.theme.border_style == BORDERSTYLE_3D)
    {
        image_rect3d(&buff, listbox->base.bounds.x, listbox->base.bounds.y, listbox->base.bounds.width, listbox->base.bounds.height, true, 
    ...
    rect_t itembounds = (rect_t) { .x = listbox->base.bounds.x + 2, .y = listbox->base.bounds.y + 2, .width = listbox->base.bounds.width - 3, .height = font_geth(listbox->base.theme.font) };
    for (uint32_t i = 0; i < listbox->item_count; i++)
    {
