...
void gui_draw_label(gui_label_t* label)
{
    ...
    image_rectf(&buff, label->base.bounds.x, label->base.bounds.y, label->base.bounds.width, label->base.bounds.height, label->base.theme.colors[THEME_BG]);

    if (label->base.theme.border_style == BORDERSTYLE_3D)
    {
        image_rect3d(&buff, label->base.bounds.x, label->base.bounds.y, label->base.bounds.width, label->base.bounds.height, label->base.flags.down, 
    ...
    if (label->base.text != NULL)
    {
        ...
        { 
            ...
            image_string(&buff, label->base.bounds.x + 4, label->base.bounds.y + 4, label->base.text, label->base.theme.colors[THEME_FG], COL32_TRANSPARENT, *label->base.theme.font);
        }
        else
        {
            image_string(&buff, label->base.bounds.x + ((label->base.bounds.width / 2) - (tw / 2)), label->base.bounds.y + ((label->base.bounds.height / 2) - (th / 2)), 
